...
#             top5 = dict(sorted(epoch_data.iteritems(), key=operator.itemgetter(1), reverse=True)[:5])
            top5 = sorted(epoch_data.items(), key=operator.itemgetter(1), reverse=True)[:5]
...
def main():
    charades_classes = load_charades_classes('datasets/Charades/Charades_v1_classes_new.txt')
